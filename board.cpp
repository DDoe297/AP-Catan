...
bool Board::isPointConnectedToPlayerRoad(Point *point, Player *owner)
{
    ...
    for(auto road:roads){
        if((road->getStartPoint()==point||road->getEndPoint()==point)&&road->getOwner()==owner){
            return true;
        }
    }
    return false;
...
{
    for(auto road:roads){
        if((road->getStartPoint()==pointOne&&road->getEndPoint()==pointTwo)||
           (road->getEndPoint()==pointOne&&road->getStartPoint()==pointTwo)){
            return true;
        }
    }
    return false;
...
bool Board::isRoadConnectedToAPiece(Point *startPoint, Point *endPoint, Player *owner)
{
    if((startPoint->getPiece()!=nullptr && startPoint->getPiece()->getOwner()==owner)||
       (endPoint->getPiece()!=nullptr && endPoint->getPiece()->getOwner()==owner)){
        return true;
    }
    for(auto road:roads){
        if(road->getStartPoint()==startPoint||
           road->getEndPoint()==startPoint){
            ...
            if(endPoint->getPiece()==nullptr||(endPoint->getPiece()->getOwner()==owner)){
                 return true;
            }
        }
    }
            ...
            if(point->getPiece()!=nullptr){
                ...
                if(settelment!=nullptr){
                    ...
                    for(auto tile:tiles){
                        ...
                        if(tile->getNumber()==roll){
                            ResourceCard type=TileTypeToResource[tile->getType()];
                            city->getOwner()->addCards({type,type});
                        }
                    }
                }
            }
...
const QVector<Road *> &Board::getRoads() const
{
    return roads;
}

const QVector<Harbor *> &Board::getHarbors() const
{
    return harbors;
}
...
Road *Board::getRoad(Point *startPoint, Point *endPoint)
{
    for(auto road:roads){
        if((road->getStartPoint()==startPoint && road->getEndPoint()==endPoint)||
           (road->getStartPoint()==endPoint && road->getEndPoint()==startPoint)){
            return road;
        }
    }
    return nullptr;
...
bool Board::checkSixAndEightAdjacency(QVector<int> nums)
{
    for(int i=0;i<nums.length()-1;i++){
        if((nums[i]==6&&nums[i+1]==8) ||(nums[i]==8&&nums[i+1]==6)){
            return true;
        }
    }
    return false;
...
const QVector<QVector<Point *>> &Board::getPoints() const
{
    return points;
}

const QVector<QVector<Tile *>> &Board::getTiles() const
{
    return tiles;
}
...
QVector<Player *> Board::playersWhoOwnSettelmentAroundTile(Tile *tile)
{
    ...
    for(auto point:points){
        if(point->getPiece()!=nullptr && !players.contains(point->getPiece()->getOwner())){
